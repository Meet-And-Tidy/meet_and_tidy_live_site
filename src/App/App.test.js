...
describe('App', () => {
  ...
  describe('token is present', () => {
    ...
    test('can render the protected route', async () => {
      ...
      expect(screen.getByText('You are logged in so you can see this page')).toBeTruthy();
  ...
  test('should handle the logout message', () => {
    ...
    render(<App />);
    dispatchEvent(new CustomEvent('token-change-message', { detail: { this: 'thing' } }));
    expect(Storage.prototype.setItem).toHaveBeenCalledWith('logout-message', '{"this":"thing"}');
  });
  ...
  test('should handle a token storageEvent', () => {
    ...
    expect(Storage.prototype.setItem).toHaveBeenCalledWith('token', 'someToken');
  });
